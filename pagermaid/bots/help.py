...
@listener(is_plugin=False, incoming=True, command="help",
...
async def help_command(context):
    ...
    if context.arguments:
        if context.arguments in help_messages:
            await context.reply(str(help_messages[context.arguments]))
        else:
...
@listener(is_plugin=False, incoming=True, command="help_raw",
          description=lang('help_des'),
          parameters=f"<{lang('command')}>")
async def help_raw_command(context):
    ...
    if context.arguments:
        ...
        for command in sorted(help_messages, reverse=False):
            result += "`" + str(command)
            result += "`, "
        await context.reply(result[:-2] + f"\n**{lang('help_send')} \"-help <{lang('command')}>\" {lang('help_see')}** "
                                          f"[{lang('help_source')}](https://t.me/PagerMaid_Modify)")


@listener(is_plugin=False, incoming=True, owners_only=True, command="lang",
...
async def lang_change(context):
    ...
    if to_lang in ldir:
        ...
        with open('config.yml', 'w') as f:
            f.write(file)
        await context.reply(f"{lang('lang_change_to')} {to_lang}, {lang('lang_reboot')}")
...
@listener(is_plugin=False, incoming=True, owners_only=True, command="alias",
...
async def alias_commands(context):
    ...
    if len(context.parameter) == 0:
        ...
        if not len(source_commands) == 0:
            ...
            await context.reply(lang('alias_list') + '\n\n' + '\n'.join(texts))
        else:
            await context.reply(lang('alias_no'))
        ...
        try:
            ...
            await context.reply(lang('alias_success'))
            await context.client.disconnect()
        except KeyError:
            await context.reply(lang('alias_no_exist'))
            return
    elif len(context.parameter) == 3:
        source_command = context.parameter[1]
        to_command = context.parameter[2]
        if to_command in help_messages:
            await context.reply(lang('alias_exist'))
            return
